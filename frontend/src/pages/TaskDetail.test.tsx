...
describe("TaskDetail", () => {
  beforeEach(() => {
    ...
    (taskService.getTaskCode as ReturnType<typeof vi.fn>).mockResolvedValue(
      mockCodeFiles,
    );
    ...
    (
      documentService.generateDocument as ReturnType<typeof vi.fn>
    ).mockResolvedValue({ id: "doc-1", status: "generating" });
  });
  ...
  describe("tabs", () => {
    ...
    it("switches to practice tab when clicked", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/실습 문제가 곧 제공됩니다/i),
        ).toBeInTheDocument();
      });
    });
  ...
  describe("document tab", () => {
    it("shows placeholder content for document generation", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/학습 문서가 준비되었습니다/i),
        ).toBeInTheDocument();
      });
    });
  });
  ...
  describe("loading state", () => {
    it("shows loading skeleton while fetching task", async () => {
      ...
      expect(
        screen.queryByText("JavaScript Async/Await"),
  ...
  describe("document status rendering", () => {
    it("shows generating status when document is generating", async () => {
      ...
      (
        documentService.getDocument as ReturnType<typeof vi.fn>
      ).mockResolvedValue(generatingDoc);
