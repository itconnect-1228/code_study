...
describe("ProjectDetail with Tasks", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (projectService.getProject as ReturnType<typeof vi.fn>).mockResolvedValue(
      mockProject,
    );
  ...
  describe("task creation", () => {
    it("renders create task button", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByRole("button", { name: /새 태스크/i }),
        ).toBeInTheDocument();
      });
    });
    ...
    it("creates task and refreshes list on submit", async () => {
      ...
      (taskService.createTask as ReturnType<typeof vi.fn>).mockResolvedValue(
        newTask,
      );
  ...
  describe("loading state", () => {
    it("shows loading skeleton while fetching tasks", async () => {
      ...
      (taskService.getTasks as ReturnType<typeof vi.fn>).mockImplementation(
        () =>
          new Promise((resolve) =>
            setTimeout(() => resolve({ tasks: mockTasks, total: 2 }), 100),
          ),
      );
