...
describe('apiClient', () => {
  ...
  it('should have request interceptors registered', () => {
    ...
    expect(
      (apiClient.interceptors.request as unknown as { handlers: unknown[] }).handlers.length
    ).toBeGreaterThan(0)
  })
  ...
  it('should have response interceptors registered', () => {
    ...
    expect(
      (apiClient.interceptors.response as unknown as { handlers: unknown[] }).handlers.length
    ).toBeGreaterThan(0)
  })
})
