...
import { taskService, type TaskCreateData } from "./task-service";
...
describe("taskService", () => {
  ...
  describe("createTask", () => {
    it("creates a task with file upload", async () => {
      ...
      const file = new File(['console.log("hello")'], "test.js", {
  ...
  describe("updateTask", () => {
    it("updates a task title", async () => {
      ...
      expect(mockApiClient.patch).toHaveBeenCalledWith("/tasks/task-1", {
        title: "Updated Title",
      });
