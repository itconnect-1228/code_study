...
describe('FileUpload', () => {
  ...
  describe('file picker', () => {
    ...
    it('supports multiple file selection', async () => {
      render(<FileUpload onFileSelect={mockOnFileSelect} />)
      const file1 = new File(['content1'], 'test1.ts', { type: 'text/typescript' })
      const file2 = new File(['content2'], 'test2.ts', { type: 'text/typescript' })
      const fileInput = document.querySelector('input[type="file"]') as HTMLInputElement
  ...
  describe('file type filtering', () => {
    it('accepts files with allowed types', async () => {
      render(<FileUpload onFileSelect={mockOnFileSelect} acceptedTypes={['.ts', '.tsx', '.js', '.jsx']} />)
    ...
    it('sets accept attribute on file input', () => {
      render(<FileUpload onFileSelect={mockOnFileSelect} acceptedTypes={['.ts', '.tsx']} />)
      const fileInput = document.querySelector('input[type="file"]') as HTMLInputElement
  ...
  describe('file size validation', () => {
    it('rejects files exceeding maxSize', async () => {
      const onError = vi.fn()
      render(<FileUpload onFileSelect={mockOnFileSelect} maxSize={1024} onError={onError} />)
      const largeContent = 'x'.repeat(2048)
      const file = new File([largeContent], 'large.ts', { type: 'text/typescript' })
      const fileInput = document.querySelector('input[type="file"]') as HTMLInputElement
      fireEvent.change(fileInput, { target: { files: [file] } })
      await waitFor(() => {
