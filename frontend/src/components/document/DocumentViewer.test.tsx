...
describe('DocumentViewer', () => {
  const sampleDocument: LearningDocument = {
    ...
    chapters: {
      ...
      lineByLine: {
        title: '라인별 설명',
        explanations: [
          { lineNumber: 1, code: 'function hello() {', explanation: '함수 선언' },
      ...
      exercises: {
        title: '연습 문제',
        items: [{ question: '다른 메시지를 출력하세요.', hint: 'console.log 사용' }],
      },
    },
  ...
  describe('2단 레이아웃', () => {
    it('CodePanel과 ExplanationPanel이 나란히 렌더링되어야 함', () => {
      render(<DocumentViewer document={sampleDocument} codeFile={sampleCodeFile} />)
    ...
    it('Monaco Editor가 코드를 표시해야 함', () => {
      ...
      expect(screen.getByTestId('monaco-editor')).toHaveTextContent('function hello()')
    })
