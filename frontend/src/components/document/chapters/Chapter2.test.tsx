...
describe('Chapter2Prerequisites', () => {
  const sampleChapter: DocumentChapters['prerequisites'] = {
    ...
    concepts: [
      {
        name: '함수',
        explanation: '재사용 가능한 코드 블록입니다. 특정 작업을 수행하고 결과를 반환할 수 있습니다.',
        analogy: '레시피와 같습니다. 한 번 정의하면 여러 번 사용할 수 있습니다.',
      },
      {
        name: '변수',
        explanation: '데이터를 저장하는 컨테이너입니다. 값을 할당하고 나중에 참조할 수 있습니다.',
        analogy: '라벨이 붙은 상자와 같습니다. 상자에 물건을 넣고 라벨로 찾을 수 있습니다.',
      },
      {
        name: 'console.log',
  ...
  describe('렌더링', () => {
    it('챕터 타이틀이 렌더링되어야 함', () => {
      ...
      expect(screen.getByRole('heading', { name: /사전 지식/i })).toBeInTheDocument()
    })
    ...
    it('각 개념 카드에 설명이 포함되어야 함', () => {
      ...
      expect(screen.getByText(/데이터를 저장하는 컨테이너/)).toBeInTheDocument()
    })
  ...
  describe('스타일링', () => {
    it('컨테이너에 기본 클래스가 적용되어야 함', () => {
      const { container } = render(<Chapter2Prerequisites data={sampleChapter} />)
