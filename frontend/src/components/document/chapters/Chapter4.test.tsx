...
describe("Chapter4LineByLine", () => {
    ...
    explanations: [
      {
        ...
        code: "function hello() {",
        explanation:
          '함수를 선언합니다. "hello"라는 이름을 가진 함수를 정의하기 시작합니다.',
      },
      {
        lineNumber: 2,
      ...
      },
      {
        ...
        code: "hello();",
        explanation:
          "위에서 정의한 hello 함수를 호출합니다. 이때 콘솔에 메시지가 출력됩니다.",
      },
    ],
  };
  ...
  describe("렌더링", () => {
    it("챕터 타이틀이 렌더링되어야 함", () => {
      ...
      expect(
        screen.getByRole("heading", { name: /라인별 설명/i }),
      ).toBeInTheDocument();
    });
    ...
    it("각 라인의 설명이 표시되어야 함", () => {
      ...
      expect(
        screen.getByText(/콘솔에 "Hello, World!"라는 메시지/),
      ).toBeInTheDocument();
    });
  ...
  describe("라인 선택 기능", () => {
    it("라인을 클릭하면 onLineSelect 콜백이 호출되어야 함", async () => {
      ...
      render(
        <Chapter4LineByLine data={sampleChapter} onLineSelect={onLineSelect} />,
      );
  ...
  describe("접근성", () => {
    it("각 라인 항목이 접근 가능한 버튼으로 렌더링되어야 함", () => {
      render(
        <Chapter4LineByLine data={sampleChapter} onLineSelect={vi.fn()} />,
      );
