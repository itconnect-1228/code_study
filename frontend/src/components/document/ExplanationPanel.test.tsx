...
const sampleDocument: LearningDocument = {
  ...
  chapters: {
      ...
      concepts: [
        {
          ...
          analogy: '레시피와 같습니다. 한 번 정의하면 여러 번 사용할 수 있습니다.',
        },
        {
          name: '변수',
    ...
    lineByLine: {
      title: '라인별 설명',
      explanations: [
        { lineNumber: 1, code: 'function hello() {', explanation: '함수를 선언합니다.' },
        { lineNumber: 2, code: 'console.log("Hello!");', explanation: '콘솔에 메시지를 출력합니다.' },
    ...
    commonPatterns: {
      title: '자주 쓰는 패턴',
      patterns: [
        { name: '함수 정의 패턴', description: '함수를 정의하고 호출하는 기본 패턴' },
      ],
    },
    exercises: {
      title: '연습 문제',
      items: [
        { question: '다른 메시지를 출력하는 함수를 만들어보세요.', hint: 'console.log 내용을 변경하세요.' },
      ],
    },
  },
...
describe('ExplanationPanel', () => {
  ...
  describe('렌더링', () => {
    it('7개의 챕터 네비게이션 탭이 렌더링되어야 함', () => {
      ...
      expect(screen.getByRole('tab', { name: /사전 지식/i })).toBeInTheDocument()
      expect(screen.getByRole('tab', { name: /핵심 로직/i })).toBeInTheDocument()
    ...
    it('첫 번째 챕터의 콘텐츠가 표시되어야 함', () => {
      ...
      expect(screen.getByText(/이 코드는 간단한 Hello World 프로그램입니다/)).toBeInTheDocument()
    })
  })
  ...
  describe('챕터 네비게이션', () => {
    ...
    it('onChapterChange 콜백이 호출되어야 함', async () => {
      ...
      render(<ExplanationPanel document={sampleDocument} onChapterChange={onChapterChange} />)
  ...
  describe('현재 챕터 하이라이트', () => {
    it('현재 선택된 챕터가 시각적으로 구분되어야 함', () => {
      render(<ExplanationPanel document={sampleDocument} currentChapter="prerequisites" />)
    ...
    it('선택되지 않은 챕터는 비활성 상태여야 함', () => {
      render(<ExplanationPanel document={sampleDocument} currentChapter="summary" />)
  ...
  describe('스타일링', () => {
    it('컨테이너에 기본 클래스가 적용되어야 함', () => {
      const { container } = render(<ExplanationPanel document={sampleDocument} />)
